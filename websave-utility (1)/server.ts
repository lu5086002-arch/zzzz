...
app.get("/api/apps", (req, res) => {
  const db = getDB();
  res.json({ success: true, list: db.marketItems || [] });
});

app.get("/api/apps/:id", (req, res) => {
  const db = getDB();
  const appItem = (db.marketItems || []).find((a: any) => a.id === req.params.id);
  if (!appItem) {
...
app.post("/api/apps/install", (req, res) => {
  const { id } = req.body;
  const db = getDB();
  const list = db.marketItems || [];
  const appIndex = list.findIndex((a: any) => a.id === id);
  if (appIndex === -1) {
    return res.status(404).json({ error: "App not found in market" });
  }
  list[appIndex].installed = true;
  db.marketItems = list;
  
  // Log it
  const auditLogs = db.auditLogs || [];
  auditLogs.unshift({
    id: 'AL_' + Math.floor(Math.random() * 10000),
    tenantId: 't_retail', // standard tenant
    userId: 'Platform Merchant Operator',
    action: 'APP_INSTALL',
    resourceType: 'app_market',
    resourceId: id,
    beforeJson: '{"installed": false}',
    afterJson: '{"installed": true}',
    createdAt: new Date().toISOString()
  });
  db.auditLogs = auditLogs;
  saveDB(db);

  res.json({ success: true, app: list[appIndex], message: "App installed successfully." });
});
...
app.post("/api/apps/uninstall", (req, res) => {
  ...
  auditLogs.unshift({
    ...
    resourceType: 'app_market',
    resourceId: id,
    beforeJson: '{"installed": true}',
    afterJson: '{"installed": false}',
    createdAt: new Date().toISOString()
  });
  db.auditLogs = auditLogs;
  saveDB(db);

  res.json({ success: true, app: list[appIndex], message: "App uninstalled successfully." });
});
...
app.post("/api/apps/upgrade", (req, res) => {
  ...
  const db = getDB();
  const list = db.marketItems || [];
  const appItem = list.find((a: any) => a.id === id);
  if (!appItem) {
    return res.status(404).json({ error: "App not found" });
  }
  // Change version rating for visual indication
  appItem.version = "v2.0.0-PRO";
  ...
  auditLogs.unshift({
    ...
    resourceType: 'app_market',
    resourceId: id,
    beforeJson: '{"version": "v1.0.0"}',
    afterJson: '{"version": "v2.0.0-PRO"}',
    createdAt: new Date().toISOString()
  });
  db.auditLogs = auditLogs;
  saveDB(db);

  res.json({ success: true, app: appItem, message: "App upgraded to professional enterprise track." });
});
...
app.get("/api/apps/license", (req, res) => {
  ...
  res.json({
    ...
    license: {
      appId: id,
      status: appItem.installed ? "active" : "unlicensed",
      key: `LIC_ECOS_` + String(id).toUpperCase() + `_` + Math.floor(Math.random() * 1000000),
      tier: "Enterprise Unlimited Plan",
      expiry: "2027-12-31T23:59:59.000Z",
      pricingModel: "recurring_usage"
    }
  });
});

app.post("/api/apps/review", (req, res) => {
  const { id, rating, reviewText, username } = req.body;
  const db = getDB();
  const list = db.marketItems || [];
  const appItem = list.find((a: any) => a.id === id);
  if (!appItem) {
  ...
  // Update app rating
  const currentRating = appItem.rating || 4.5;
  appItem.rating = parseFloat(((currentRating * 9 + parseFloat(rating || 5)) / 10).toFixed(1));
  
  // Custom Reviews field if not exists
  if (!appItem.reviewsList) {
    appItem.reviewsList = [];
  }
  appItem.reviewsList.unshift({
    id: 'REV_' + Math.floor(Math.random() * 10000),
    username: username || "Merchant_Ecos",
    rating: parseFloat(rating),
    comment: reviewText || "Fully responsive layout. Seamless enterprise sync",
    createdAt: new Date().toISOString()
  });
...
app.post("/api/developer/app/create", (req, res) => {
  const { name, description, category, webhookUrl, scopes } = req.body;
  if (!name) {
    return res.status(400).json({ error: "Missing required 'name' field." });
  }
  const db = getDB();
  if (!(db as any).developerApps) {
    (db as any).developerApps = [];
  }
  
  const newDevApp = {
    id: 'dapp_' + Math.floor(Math.random() * 100000),
    name,
    description: description || "Enterprise extension pack for ECOS OS",
    category: category || "Utility",
    clientId: 'client_id_ecos_' + Math.floor(Math.random() * 10000000).toString(16),
    clientSecret: 'client_secret_ecos_' + Math.floor(Math.random() * 10000000).toString(16) + Math.floor(Math.random() * 10000000).toString(16),
    webhookUrl: webhookUrl || "",
    scopes: scopes || ["products:read", "orders:read"],
    environment: "sandbox",
    status: "active",
    published: false,
    createdAt: new Date().toISOString()
  ...
  (db as any).developerApps.unshift(newDevApp);

  // Add developer log
  if (!(db as any).developerLogs) {
    (db as any).developerLogs = [];
  }
  (db as any).developerLogs.unshift({
    id: 'DLOG_' + Math.floor(Math.random() * 100000),
    timestamp: new Date().toISOString(),
    type: "REST_API_KEY_REGISTRATION",
    endpoint: "/api/developer/app/create",
    method: "POST",
    statusCode: 201,
    details: `Custom developer app 「${name}」 created successfully, Client ID generated.`
  });

  saveDB(db);
  res.status(201).json({ success: true, app: newDevApp });
});

app.get("/api/developer/app/list", (req, res) => {
  const db = getDB();
  const list = (db as any).developerApps || [];
  res.json({ success: true, list });
});

app.post("/api/developer/webhook", (req, res) => {
  const { appId, eventType, webhookUrl } = req.body;
  const db = getDB();
  const list = (db as any).developerApps || [];
  const appItem = list.find((a: any) => a.id === appId);
  if (!appItem) {
    return res.status(404).json({ error: "Developer app not found." });
  }
  
  appItem.webhookUrl = webhookUrl;
  if (!appItem.subscribedWebhooks) {
    appItem.subscribedWebhooks = [];
  }
  if (!appItem.subscribedWebhooks.includes(eventType)) {
    appItem.subscribedWebhooks.push(eventType);
  }
  ...
  (db as any).developerLogs.unshift({
    ...
    endpoint: "/api/developer/webhook",
    method: "POST",
    statusCode: 200,
    details: `Webhook config changed for ${appItem.name}: Listening for event 「${eventType}」`
  });

  saveDB(db);
  res.json({ success: true, app: appItem, message: "Webhook subscription registered and tested successfully." });
});

app.get("/api/developer/logs", (req, res) => {
  const db = getDB();
  let logs = (db as any).developerLogs || [];
  if (logs.length === 0) {
    logs = [
      { id: 'DLOG_9921', timestamp: new Date(Date.now() - 50000).toISOString(), type: "REST_API", endpoint: "/api/v1/products", method: "GET", statusCode: 200, details: "Query active inventory catalog for POS synchronization." },
      { id: 'DLOG_9922', timestamp: new Date(Date.now() - 150000).toISOString(), type: "WEBHOOK_TRIGGER", endpoint: "https://your-server.com/webhooks/order-paid", method: "POST", statusCode: 200, details: "Client payload dispatched successfully. Delivery time: 142ms." },
      { id: 'DLOG_9923', timestamp: new Date(Date.now() - 320000).toISOString(), type: "OAUTH_AUTHORIZE", endpoint: "/api/developer/oauth/authorize", method: "GET", statusCode: 302, details: "Authorization code grant issued safely under JWT signature." },
      { id: 'DLOG_9924', timestamp: new Date(Date.now() - 450000).toISOString(), type: "REST_API", endpoint: "/api/v1/orders/ORD-9839", method: "GET", statusCode: 200, details: "Retrieved single resource with high isolation safety check." }
    ];
    (db as any).developerLogs = logs;
    saveDB(db);
  }
...
app.post("/api/developer/publish", (req, res) => {
  const { appId, author, priceText, categoryName } = req.body;
  const db = getDB();
  const list = (db as any).developerApps || [];
  const appItem = list.find((a: any) => a.id === appId);
  if (!appItem) {
  ...
  appItem.published = true;

  // Add item to market
  const marketItems = db.marketItems || [];
  const alreadyInMarket = marketItems.some((m: any) => m.id === appId);
  if (!alreadyInMarket) {
    marketItems.unshift({
      id: appId,
      name: appItem.name,
      developer: author || "Enterprise developer",
      icon: "Sparkles",
      price: priceText || "$19/mo",
      rating: 5.0,
      category: categoryName || "Utility",
      description: appItem.description,
      installed: false
    });
  }
  db.marketItems = marketItems;
  ...
  (db as any).developerLogs.unshift({
    id: 'DLOG_' + Math.floor(Math.random() * 100000),
    timestamp: new Date().toISOString(),
    type: "APP_PUBLISHED_TO_MARKET",
    endpoint: "/api/developer/publish",
    method: "POST",
    statusCode: 201,
    details: `App 「${appItem.name}」 compiled, bundled and hot-deployed to the Enterprise App Marketplace!`
  });

  saveDB(db);
  res.json({ success: true, app: appItem, message: "App successfully compiled and deployed to public marketplace." });
});
