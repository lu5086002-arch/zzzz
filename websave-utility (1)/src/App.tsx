...
import AppMarket from './components/merchant_client/AppMarket';
import DeveloperCenter from './components/merchant_client/DeveloperCenter';
...
export default function App() {
  ...
  if (adminMode === 'super_admin') {
    return (
      <div id="saas-platform-root" className="flex h-screen w-full bg-[#030303] text-slate-200 overflow-hidden font-sans">
        ...
        <div className="fixed top-2.5 right-4 z-[99999] flex items-center gap-1.5 bg-slate-950/95 hover:bg-slate-900 text-white text-[10px] px-2.5 py-1 rounded-full border border-slate-800 shadow-2xl backdrop-blur-md transition-all select-none font-sans">
          <div className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse shadow-[0_0_8px_#22d3ee]" />
          <span className="font-mono text-zinc-500 text-[9px]">Dev Bridge:</span>
          <span className="font-black text-white tracking-tight text-[9px]">SaaS 平台总后台 🦾</span>
          ...
          <button
            ...
            className="px-2 py-0.5 bg-slate-800 hover:bg-[#10b981] hover:text-white text-[#10b981] font-extrabold rounded-full transition-all duration-200 active:scale-95 leading-none text-[9px] cursor-pointer"
          >
            切到 商家端 👕
          </button>
        </div>
  ...
  if (showMobilePwa) {
    return (
      <div className="w-full h-screen overflow-hidden flex flex-col bg-slate-100 relative">
        ...
        <div className="fixed top-2.5 right-4 z-[99999] flex items-center gap-1.5 bg-slate-950/95 hover:bg-slate-950 text-white text-[10px] px-2.5 py-1 rounded-full border border-slate-800/80 shadow-2xl backdrop-blur-md transition-all select-none scale-90 sm:scale-100 origin-top-right">
          <div className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse shadow-[0_0_8px_#22d3ee]" />
          <span className="font-mono text-zinc-500 text-[9px]">Dev Bridge:</span>
          <span className="font-black text-white tracking-tight text-[9px]">SaaS 移动端 👕</span>
          <div className="w-px h-2.5 bg-zinc-800 mx-0.5" />
          <button
            onClick={() => {
              setAdminMode('super_admin');
            ...
            className="px-2 py-0.5 bg-slate-800 hover:bg-[#07C2E3] hover:text-slate-100 text-[#07C2E3] font-extrabold rounded-full transition-all duration-200 active:scale-95 leading-none text-[9px] cursor-pointer"
          >
            切到总后台 🦾
          </button>
          <div className="w-px h-2.5 bg-zinc-800 mx-0.5" />
          <select
            ...
            className="bg-slate-800 text-[#07C2E3] border border-slate-700 rounded text-[9px] p-0.5 font-bold focus:outline-none cursor-pointer"
          >
            <option value="responsive">📱 自适应</option>
            <option value="desktop">💻 桌面</option>
            <option value="mobile_pwa">📱 PWA</option>
          </select>
        </div>
      </div>
    );
  ...
  return (
    <div id="saas-platform-root" className="flex h-screen w-full bg-[#F9FAFB] font-sans text-slate-900 overflow-hidden">
        ...
        <div id="saas-split-pane" className="flex-1 flex overflow-hidden">
          ...
          <div id="saas-workspace" className="flex-1 overflow-y-auto p-6 lg:p-8 space-y-6">
            ...
            {activeTab === 'marketplace' && (
              <MarketplaceIntegratedLayout addLog={addLog} />
            )}
      ...
      {/* Dev Mode Environment Bridge - Secure Sandbox Teleportation Tunnel */}
      <div className="fixed top-2.5 right-4 z-[99999] flex items-center gap-1.5 bg-slate-950/95 hover:bg-slate-950 text-white text-[10px] px-2.5 py-1 rounded-full border border-slate-800/85 shadow-2xl backdrop-blur-md transition-all sm:scale-100 scale-90 select-none font-sans max-w-[90vw] origin-top-right">
        <div className="flex items-center gap-1 shrink-0">
          <div className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse shadow-[0_0_8px_#22d3ee]" />
          <span className="font-mono text-slate-400 text-[9px]">Dev Bridge:</span>
          <span className="font-black text-white tracking-tight shrink-0 text-[9px]">{adminMode === 'merchant' ? 'SaaS 商家端 👕' : 'SaaS 后台 🦾'}</span>
        </div>
        <div className="w-px h-2.5 bg-zinc-805 mx-0.5" />
        <button
          onClick={() => {
          ...
          className="px-2 py-0.5 bg-slate-800 hover:bg-[#07C2E3] hover:text-slate-950 text-[#07C2E3] font-extrabold rounded-full transition-all duration-200 active:scale-95 leading-none text-[9px] cursor-pointer"
        >
          切换环境 ⚡
        </button>
        {adminMode === 'merchant' && (
...
function MarketplaceIntegratedLayout({ addLog }: { addLog: any }) {
  const [subTab, setSubTab] = useState<'market' | 'developer'>('market');
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex border-b border-slate-200 select-none">
        <button
          ...
          className={`pb-3 px-6 text-xs font-black transition-all cursor-pointer flex items-center gap-2 border-b-2 ${subTab === 'market' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-800'}`}
        >
          🏬 应用市场 (App Market)
        </button>
        <button
          onClick={() => setSubTab('developer')}
          className={`pb-3 px-6 text-xs font-black transition-all cursor-pointer flex items-center gap-2 border-b-2 ${subTab === 'developer' ? 'border-sn-600 text-indigo-600 border-[#07C2E3]' : 'border-transparent text-slate-500 hover:text-slate-800'}`}
        >
          🛠️ 开发者开放中枢 (Developer Center)
        </button>
      </div>

      <div>
        {subTab === 'market' ? (
          <AppMarket onAddLog={addLog} />
        ) : (
          <DeveloperCenter onAddLog={addLog} />
        )}
      </div>
    </div>
  );
